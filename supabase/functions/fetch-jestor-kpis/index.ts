...
serve(async (req) => {
  ...
  try {
    ...
    if (!equipe?.jestor_api_token) {
      return new Response(JSON.stringify({ error: "Jestor API token not configured" }), {
    ...
    const reunioesAgendadas = currentMonthLeads.filter(
      (lead: any) => lead.status === "Agendada" || lead.status === "agendada",
    ).length;
    const negociosFechados = currentMonthLeads.filter(
      (lead: any) => lead.status === "Fechado" || lead.status === "fechado",
    ).length;
    ...
    const taxaConversaoNegocio =
      reunioesAgendadas > 0 ? ((negociosFechados / reunioesAgendadas) * 100).toFixed(1) : "0.0";
